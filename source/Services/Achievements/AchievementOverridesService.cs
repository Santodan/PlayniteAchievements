...
namespace PlayniteAchievements.Services.Achievements
{
    public sealed class AchievementOverridesService
    {
        ...
        {
            var overrides = PlayniteAchievementsPlugin.Instance?.Settings?.Persisted?.PreferredProviderOverrides;
            return overrides != null &&
                   overrides.TryGetValue(playniteGameId, out var providerKey) &&
        ...
        public CacheWriteResult SetPreferredProviderOverride(Guid playniteGameId, string providerKey)
        {
            if (playniteGameId == Guid.Empty)
            {
                return CacheWriteResult.CreateFailure(string.Empty, "invalid_game_id", "Invalid game id.");
            }

            var persisted = PlayniteAchievementsPlugin.Instance?.Settings?.Persisted;
            if (persisted == null)
            {
                return CacheWriteResult.CreateFailure(playniteGameId.ToString(), "settings_unavailable", "Settings are unavailable.");
            }

            persisted.PreferredProviderOverrides ??= new Dictionary<Guid, string>();
            providerKey = providerKey?.Trim();
            if (string.IsNullOrWhiteSpace(providerKey))
            {
                persisted.PreferredProviderOverrides.Remove(playniteGameId);
            }
            else
            {
                persisted.PreferredProviderOverrides[playniteGameId] = providerKey;
            }

            _cacheService.NotifyCacheInvalidated(new[] { playniteGameId });
            return CacheWriteResult.CreateSuccess(playniteGameId.ToString(), DateTime.UtcNow);
        }
