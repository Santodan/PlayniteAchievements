...
namespace PlayniteAchievements.Services.Cache
{
    ...
    public sealed class CacheManager : ICacheManager, ICacheReadOptimizations, IAchievementFilterMirror, IFriendCacheManager, IInGameProgressCacheWriter, IDisposable
    {
        ...
        // Compatibility overload for fork providers. The v3 cache stores one resolved
        // achievement payload per game key, so provider selection is already represented
        // by GameAchievementData.ProviderKey.
        public GameAchievementData LoadGameData(string key, string providerKey)
        {
